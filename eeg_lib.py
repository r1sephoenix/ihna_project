...
def predict_lgbm(data, eeg_param, ps=None):
    ...
    m = lgbm(**fit_params)
    param_grid = {'reg_alpha': list(np.power(10.0, np.arange(-5, 5))),
...
def plot_clusters(data, l_ind, cl_param):
    ...
    for i, co, name in zip(range(3), [0.1, 0.38, 0.66], ['A', 'B', 'C']):
        t_obs, clusters, cluster_p_values, _ = permutation_cluster_test([data[l_ind[0], i, ...],
                                                                         data[l_ind[1], i, ...]],
                                                                        threshold=thr,
                                                                        adjacency=None, out_type='mask',
                                                                        n_permutations=1024, verbose=False)
        ...
        for j, key in enumerate(list(cl_param[0].keys())):
            mne.viz.plot_topomap(res_ar[j], cl_param[1], vmin=vmin, vmax=vmax, axes=axes[i, j],
                                 show=False, cmap='hot', mask=mr_ar[j].reshape(-1, 1))
            ...
            axes[i, 0].text(-0.2, 0, f'{name}', fontdict={'fontsize': 50, 'fontweight': 'semibold'})
    return fig

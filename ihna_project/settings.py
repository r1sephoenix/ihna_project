...
class EEGSettingsMathTask:
    ...
    channels_to_drop: List[str] = field(
    ...
    fr_bands: Dict[str, List[int]] = field(
        default_factory=lambda: {
            "theta1": [4, 6],
            "theta2": [6, 8],
            "alpha1": [8, 10],
    ...
    dict_cls: Dict[str, List[int]] = field(
        default_factory=lambda: {
            "241/244": [0, 2],
            "242/244": [1, 2],
            "241/242": [0, 1],
    ...
    def __post_init__(self, path):
        ...
        indexes = [
            re.search("(.+?)_", file_name).group(1) for file_name in paths_to_subjects
        ]
        ...
        paths_to_edf = [
            glob.glob(path + f"/{file_name}/Reals/*.edf")
            for file_name in paths_to_subjects
        ]
        self.files = {key: value for key, value in zip(indexes, paths_to_edf)}


@dataclass
class EEGSettingsAge:
    """
    Settings for EEGs pre-and post-processing
    ...
    channels_to_drop: List[str] = field(default_factory=lambda: [])
    montage_name: str = ""
    events: List[int] = field(default_factory=lambda: [241, 242, 244])
    fr_bands: Dict[str, List[int]] = field(
        default_factory=lambda: {
    ...
    order_codes: List[str] = field(default_factory=list)
    control_codes: List[str] = field(default_factory=lambda: [])
    autistic_codes: List[str] = field(default_factory=list)
    index_control: List[int] = field(default_factory=list)
    index_autistic: List[int] = field(default_factory=list)
    index_all: List[int] = field(default_factory=list)
    files: Dict[str, List[str]] = field(default_factory=dict)
    path: InitVar[str] = ""
    ...
    def __post_init__(self, path):
        ...
        self.not_mat_codes = [x for x in indexes if x not in self.control_codes]
        self.order_codes = self.control_codes + self.autistic_codes
        self.index_control = [self.order_codes.index(i) for i in self.control_codes]
        self.index_autistic = [self.order_codes.index(i) for i in self.autistic_codes]
        self.index_all = self.index_control + self.index_autistic
        paths_to_edf = [
            glob.glob(path + f"/{file_name}/Reals/*.edf")
            for file_name in paths_to_subjects
        ]
